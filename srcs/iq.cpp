...
Description: Contains the IQ class, which simulates the operation of a
Instruction Queue.
*/
...
bool IQ::isEmpty() { return issue_queue.empty(); }
...
void IQ::initialize() { issue_queue.clear(); }
...
bool IQ::dispatchInst(Stage &stage) {
  if (this->issue_queue.size() < this->max_size) {
    ...
    if (stage.opcode == "ADD" || stage.opcode == "SUB" ||
        stage.opcode == "MUL" || stage.opcode == "AND" ||
        stage.opcode == "OR" || stage.opcode == "EX-OR") {
      if (stage.valids.at(1) && stage.valids.at(2)) {
        stage.ready = true;
      }
    } else if (stage.opcode == "MOVC" || stage.opcode == "BZ" ||
               stage.opcode == "BNZ") {
      stage.ready = true;
    ...
    } else if (stage.opcode == "BAL" || stage.opcode == "JUMP") {
...
bool IQ::issue(Stage &ALU, Stage &MUL, Stage &LSFU, Stage &B) {
  ...
  for (auto i = this->issue_queue.begin(); i != this->issue_queue.end();) {
    if (i->isReady()) {
      ...
      if (i->opcode == "ADD" || i->opcode == "SUB" || i->opcode == "AND" ||
          i->opcode == "OR" || i->opcode == "EX-OR" || i->opcode == "MUL") {
        hitArith = true;
      }
      ...
      // Arithmetic Opcodes
      if (i->opcode == "ADD" || i->opcode == "SUB" || i->opcode == "AND" ||
          i->opcode == "OR" || i->opcode == "EX-OR" || i->opcode == "MOVC") {
      ...
      if ((i->opcode == "BZ" || i->opcode == "BNZ") && !hitArith) {
        advSuccess = i->advance(B);
...
// 						sorted at all times by their timestamp of
// creation (c)
